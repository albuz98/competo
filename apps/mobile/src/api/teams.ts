...
export async function createTeam(
  ...
  if (isMocking && mockFlags.IS_MOCKING_CREATE_TEAM) {
    ...
    if (mockTeamCache && representative) {
      const duplicate = mockTeamCache.find(t =>
        ...
        t.members.some(m => m.id === representative.id && m.role === 'representative')
      );
      if (duplicate) {
  ...
  return apiFetch<Team>('/teams', { method: 'POST', body: JSON.stringify({ name, sport }) }, token);
}
...
export async function inviteMember(
  ...
  if (isMocking && mockFlags.IS_MOCKING_INVITE_MEMBER) {
    ...
    if (getMockPendingInvites().find(i => i.teamId === teamId && i.toUserId === appUser.id)) return; // already invited
...
export async function searchUsers(query: string, token: string): Promise<AppUser[]> {
  ...
  return apiFetch<AppUser[]>(`/users/search?q=${encodeURIComponent(query)}`, {}, token);
}
...
export async function acceptInvite(
  ...
  if (isMocking && mockFlags.IS_MOCKING_ACCEPT_INVITE) {
    ...
    if (!team) {
      team = {
        ...
        sport: invite.sport,
        members: [
          { id: invite.fromUserId, firstName: invite.fromFirstName, lastName: invite.fromLastName, username: `${invite.fromFirstName.toLowerCase()}`, role: 'representative' as const },
        ],
  ...
  return apiFetch<void>(`/teams/invites/${inviteId}/accept`, { method: 'POST' }, token);
}
...
export async function rejectInvite(inviteId: string, token: string): Promise<void> {
  ...
  return apiFetch<void>(`/teams/invites/${inviteId}/reject`, { method: 'POST' }, token);
}
...
export async function updateMemberRole(
  ...
  if (isMocking && mockFlags.IS_MOCKING_UPDATE_MEMBER_ROLE) {
    ...
    const member = team.members.find(m => m.id === memberId);
    if (!member) throw new Error('Membro non trovato');
    if (member.role === 'representative') throw new Error('Non puoi cambiare il ruolo del rappresentante');
    if (newRole === 'allenatore') {
      const existing = team.members.find(m => m.role === 'allenatore' && m.id !== memberId);
      if (existing) throw new Error('La squadra ha già un allenatore');
    }
    if (newRole === 'portiere') {
      const existing = team.members.find(m => m.role === 'portiere' && m.id !== memberId);
