...
export async function addFavorite(tournament: Tournament, token: string): Promise<void> {
  ...
  return apiFetch<void>(`/favorites/${tournament.id}`, { method: 'POST' }, token);
}

export async function removeFavorite(tournamentId: string, token: string): Promise<void> {
  if (isMocking && mockFlags.IS_MOCKING_REMOVE_FAVORITE) {
    mockFavoritesCache = mockFavoritesCache.filter((t) => t.id !== tournamentId);
    return;
  }
  return apiFetch<void>(`/favorites/${tournamentId}`, { method: 'DELETE' }, token);
}
